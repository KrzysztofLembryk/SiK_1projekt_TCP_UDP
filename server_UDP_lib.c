...
int UDP_server_wait_for_CONN(int socket_fd, char *buff, int buff_size, 
    int flag, struct sockaddr_in *client_address, socklen_t *client_address_len, ssize_t *read_bytes)
{
    ...
    *read_bytes = recvfrom(socket_fd, buff, buff_size,
                                    flag, 
    ...
    if (*read_bytes < 0)
    {
        syserr("UDP_server_handler - recvfrom < 0");
    }
}
...
void UDP_server_handler(int socket_fd, struct sockaddr_in *server_address)
{
    ...
    while (true)
    {
        ...
        int ret_val = UDP_server_wait_for_CONN(socket_fd, buff, BUFFOR_SIZE, DEFAULT_FLAG, &client_address, &client_address_length, &read_bytes);
